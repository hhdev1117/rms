			...
			success : function(result) {
				...
				if(result.length == 0){
					...
					for ( var i in result) {
						...
						html += "<td>" + result[i].ztrs_card_bk_no + "</td>";
						html += "</tr>";
					}
			    }
...
<h2>자동이체중복 <button class="btn-menual" id="menual" onClick="openMenualPopup();">주요지표 정의</button></h2>
<div class="filter-area">
	...
	<div class="right">
		<button class="btn-tb-top excel-1" onclick="openExcelPopup(zregion, zdept, zdept_brch, zemp, before_yyyymm, after_yyyymm, active_bcg_id, 'excelTrsDup_d.do', '[현장용]자동이체중복_리스트.xlsx');">엑셀다운</button>
		<c:if test="${zregion_gb eq 'A'}">
			<button class="btn-tb-top excel-2" onclick="openExcelPopup(zregion, zdept, zdept_brch, zemp, before_yyyymm, after_yyyymm, active_bcg_id, 'excelTrsDup_da.do', '[관리용]자동이체중복_리스트.xlsx');">전사 엑셀다운</button>
		</c:if>
	</div>
</div>
