...
<script>
	...
	$(document).ready(function(e){
		...
		openPopup();
		//alert("7월 사업운영 모니터링 실적은 현재 (21년 7월)조직 기준의 마감 데이터 이므로 주요지표의 비중 및 건 수의 값은 상이하게 보여질 수 있습니다. 이에 따라 관리 순위는 집계하지 않습니다. 현재 조직 기준 상세 데이터는 모니터링 세부 메뉴에서 확인하시기 바랍니다.\n\n[문의]\n- 사업운영 모니터링 시스템 운영 : 사업지원팀 장지원 (829-0543)\n- 사업운영 모니터링 시스템 문의 : IT사업지원팀 김시은 (829-0356)");
	});
	...
	function btnSearch() {
		...
		var brch_gb = "";
		if($("input:checkbox[id='chkbx1']").prop("checked") == false && $("input:checkbox[id='chkbx2']").prop("checked") == false) {
			brch_gb = "";
		} else if($("input:checkbox[id='chkbx1']").prop("checked") == true && $("input:checkbox[id='chkbx2']").prop("checked") == false) {
			brch_gb = "BL";
		} else if($("input:checkbox[id='chkbx1']").prop("checked") == false && $("input:checkbox[id='chkbx2']").prop("checked") == true) {
			brch_gb = "LC";
		} else {
			clickBrchCheck();
			return;
	...
	LC, BL조회
	=============================================================--%>
	function btnBrchSearch(brch_gb) {
		<%-- 
			달력, 펼쳐보기 닫기
		--%>
		$("#calFilterBox, #sumOpenGrp, #sumOpenSearchFilter").css("visibility", "hidden");
	...
	function excelDownloadForSum() {
		var chkbx1 = $("input:checkbox[id='chkbx1']").is(":checked");
		var chkbx2 = $("input:checkbox[id='chkbx2']").is(":checked");
		...
		var brch_gb = "";
		if(chkbx1 == false && chkbx2 == false) {
			brch_gb = "ALL";
			openExcelPopupForSum(zregion, zdept, zdept_brch, zemp, yyyymm, yyyymm, active_bcg_id, 'excelMoniSum_d.do', '주요지표.xlsx', brch_gb, cnt_gb);
		} else if(chkbx1 == true && chkbx2 == false) {
			brch_gb = "BL";
			openExcelPopupForSum(zregion, zdept, zdept_brch, zemp, yyyymm, yyyymm, active_bcg_id, 'excelMoniSum_ex_d.do', '주요지표.xlsx', brch_gb, cnt_gb);
		} else if(chkbx1 == false && chkbx2 == true) {
			brch_gb = "LC";
			openExcelPopupForSum(zregion, zdept, zdept_brch, zemp, yyyymm, yyyymm, active_bcg_id, 'excelMoniSum_ex_d.do', '주요지표.xlsx', brch_gb, cnt_gb);
		} else {
			alert("구분을 한개 이상 선택해주세요.");
			return;
		}
	}
	...
	function orderSearch() {
		...
		if($("#order-rank1-pointer").html() == "▼") {
			$("#order-rank1").html("관리순위<span id='order-rank1-pointer'></span>");
			btnSearch("NONE");
		} else if($("#order-rank1-pointer").html() == "▲") {
			$("#order-rank1").html("관리순위</br><span id='order-rank1-pointer' style='color : red;'>▼</span>");
			btnSearch("DESC");
		} else {
			$("#order-rank1").html("관리순위</br><span id='order-rank1-pointer' style='color : red;'>▲</span>");
			btnSearch("ASC");
	...
	function changeGrp1(e) {
		if($("input:checkbox[id='" + e.id + "']").is(":checked")) {
			if(e.id == "chkbx1") {
				$('input:checkbox[id="chkbx1"]').prop("checked", true);
				$('input:checkbox[id="chkbx2"]').prop("checked", false);
			} else {
				$('input:checkbox[id="chkbx1"]').prop("checked", false);
				$('input:checkbox[id="chkbx2"]').prop("checked", true);
			}
		}
	}
...
<div class="filter-area">
	<div class="left">
		...
		<div class="filtering select">
		    ...
		    <div class="filterBox option-box" id="sumOpenSearchFilter">
		    	<div>
					<div class="chkbx">
		    	...
		    	<div>
					<div class="chkbx">
						<input type="checkbox" id="chkbx1" name="checkbox-btn" onchange="changeGrp1(this);">
						<label for="chkbx1">방문</label> <%-- "홈러닝" -> "방문" 수정 2021.07.29 --%>
					</div>
					<div class="chkbx">
						<input type="checkbox" id="chkbx2" name="checkbox-btn" onchange="changeGrp1(this);">
						<label for="chkbx2">러닝</label>
					</div>						    		
		    	</div>
		    </div>
		</div>
		<!-- 
		<div class="filtering">
			<div class="custom-input selt-Chk" onclick="btnSumSearchFilterOpen();">
				<input type="text" id="" placeholder="선택보기" disabled>
				<a href="javascript:void(0);" class="icon_filter"><img src="${webPath}/images/icon_filter.png" alt=""></a>
			</div>
			
			<div class="filterBox" id="sumOpenSearchFilter">
				<div class="tblchbx">
					<input type="checkbox" id="ckbInd" name="ckbSelect1" onchange="setRadio(this);" checked="checked"><label for="ckbInd">비중</label>										
				</div>
				<div class="tblchbx" style="text-align: left;">
					<input type="checkbox" id="ckbCnt" name="ckbSelect2" onchange="setRadio(this);"><label for="ckbCnt">대상건수</label>										
				</div>
			</div>
		</div>
		 -->
		<button class="btn-tb-top search" id="search" onClick="btnSearch();" style="margin-left: 10px">검색</button>
		<!--
		<div class="filtering">
			<div class="custom-input selt-Chk" onclick="btnSumOpen();">
				<input type="text" id="" placeholder="펼쳐보기" disabled>
				<a href="javascript:void(0);" class="icon_filter"><img src="${webPath}/images/icon_filter.png" alt=""></a>
			</div>
			<div class="filterBox" id="sumOpenGrp">
				<div class="tblchbx">
					<input type="checkbox" id="rdo1" name="ckbSelect1" onchange="setTreeTable(this);"><label for="rdo1">사업국</label>										
				</div>
				<div class="tblchbx" style="text-align: left;">
					<input type="checkbox" id="rdo2" name="ckbSelect2" onchange="setTreeTable(this);"><label for="rdo2">팀[센터]</label>										
				</div>
			</div>
		</div>
		-->
...
<div class="tbl">
	<table id="basic">
		<colgroup>
			...
			<col style="width: 8%;">
			<col style="width: 8%;">
			<col class="notall"  style="width: 4%;" style="display:none;">
			<!-- <col class="notall"  style="width: 4%;" style="display:none;"> -->
		</colgroup>
		<thead>
			<tr>
				...
				<th>써밋제품 학습</th>
				<th>긴급교재</br>신청</th>
				<th class="notall" style="display:none;">관리순위</th>
				<!-- <th class="notall" style="display:none;">개선순위</th> -->
			</tr>
		</thead>
