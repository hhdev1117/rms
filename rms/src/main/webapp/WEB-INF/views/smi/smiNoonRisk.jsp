			...
			success : function(result) {
				...
				if(result.length == 0){
					...
					for ( var i in result) {
						...
						html += "<td>" + result[i].zcls_nm + "</td>";
						html += "<td>" + result[i].zdup_month + "</td>";
						html += "</tr>";
					}
			    }
				
				$("#smi-main-tbody").append(html);
...
<h2>써밋제품 학습 <button class="btn-menual" id="menual" onClick="openMenualPopup();">주요지표 정의</button></h2>
<div class="filter-area">
	...
	<div class="right">
		<button class="btn-tb-top excel-1" onclick="openExcelPopup(zregion, zdept, zdept_brch, zemp, before_yyyymm, after_yyyymm, active_bcg_id, 'excelSmiRisk_d.do', '[현장용]써밋제품_미학습_Risk_리스트.xlsx');">엑셀다운</button>
		<c:if test="${zregion_gb eq 'A'}">
			<button class="btn-tb-top excel-2" onclick="openExcelPopup(zregion, zdept, zdept_brch, zemp, before_yyyymm, after_yyyymm, active_bcg_id, 'excelSmiRisk_da.do', '[관리용]써밋제품_미학습_Risk_리스트.xlsx');">전사 엑셀다운</button>
		</c:if>
	</div>
</div>
	...
	<table>
		...
		<thead>
			<tr>
				...
				<th>교실명</th>
				<th>연속 미 로그인<br>개월수</th>
			</tr>
		</thead>
		<tbody id="smi-main-tbody">
			<tr>
