...
<script>
	...
	function btnSearch(page) {
		...
		}).always(function(jqXHR, testStatus){
			closeLoading();
		});
	}
</script>
<%@ include file="../com/menual.jsp"%>
<h2>2개월 체납 <button class="btn-menual" id="menual" onClick="openMenualPopup();">주요지표 정의</button></h2>
<div class="filter-area">
	...
	<div class="right">
		<button class="btn-tb-top excel-1" onclick="openExcelPopup(zregion, zdept, zdept_brch, zemp, before_yyyymm, after_yyyymm, active_bcg_id, 'excelArrPayment_d.do', '[현장용]2개월체납_리스트.xlsx');">엑셀다운</button>
		<c:if test="${zregion_gb eq 'A'}">
			<button class="btn-tb-top excel-2" onclick="openExcelPopup(zregion, zdept, zdept_brch, zemp, before_yyyymm, after_yyyymm, active_bcg_id, 'excelArrPayment_da.do', '[관리용]2개월체납_리스트.xlsx');">전사 엑셀다운</button>
		</c:if>
	</div>
</div>
