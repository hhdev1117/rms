	...
	function btnSearch(page) {
		...
		$.ajax({
			...
			success : function(result) {
				...
				if(result.length == 0){
		        	html += '<tr>';
		        	html += '<td colspan="9" style="font-size:14px;">조회결과가 없습니다.</td>';
		        	html += '</tr>';
					...
					for ( var i in result) {
						...
						if(isEmpty(result[i].zlc_nm)) {
							...
							html += "<td>" + result[i].zdept_brch_id + "[" + changeEmpIdToKunnr(result[i].zlc_nm) + "]</td>";
						}
...
<h2>이체제외 후 학습 <button class="btn-menual" id="menual" onClick="openMenualPopup();">주요지표 정의</button></h2>
<div class="filter-area">
	...
	<div class="right">
		<button class="btn-tb-top excel-1" onclick="openExcelPopup(zregion, zdept, zdept_brch, zemp, before_yyyymm, after_yyyymm, active_bcg_id, 'excelTranStudy_d.do', '[현장용]이체제외후학습_리스트.xlsx');">엑셀다운</button>
		<c:if test="${zregion_gb eq 'A'}">
			<button class="btn-tb-top excel-2" onclick="openExcelPopup(zregion, zdept, zdept_brch, zemp, before_yyyymm, after_yyyymm, active_bcg_id, 'excelTranStudy_da.do', '[관리용]이체제외후학습_리스트.xlsx');">전사 엑셀다운</button>
		</c:if>
	</div>
</div>
...
<div class="tbl">
	<table>
		...
		<tbody id="crd-main-tbody">
			<tr>
				<td colspan="9" style="font-size:14px;">조회결과가 없습니다.</td>
			</tr>
		</tbody>
	</table>
