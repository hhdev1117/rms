...
<script>
	...
	function btnSearch(page) {
		...
		$.ajax({
			type : "POST",
			dataType : "json",
			url : "${ctx}/ind/getIndAuthorization.do",
			...
			success : function(result) {
				...
				if(result.length == 0){
		        	html += '<tr>';
		        	html += '<td colspan="12" style="font-size:14px;">조회결과가 없습니다.</td>';
		        	html += '</tr>';
					...
					for ( var i in result) {
						if(result[i].zrms_ym == (result[i].zsms_send_dt).substring(0, 6)) {
							...
							html += "<td>" + result[i].zcert_gb + "</td>";
							html += "<td>" + result[i].zrec_conn + "</td>";
							html += "<td>" + result[i].zcert_name + "</td>";
							html += "<td>" + result[i].zcert_tel + "</td>";
							html += "</tr>";
...
<h2>개인정보 인증 <button class="btn-menual" id="menual" onClick="openMenualPopup();">주요지표 정의</button></h2>
<div class="filter-area">
	...
	<div class="right">
		<button class="btn-tb-top excel-1" onclick="openExcelPopup(zregion, zdept, zdept_brch, zemp, before_yyyymm, after_yyyymm, active_bcg_id, 'excelIndAuth_d.do', '[현장용]개인정보인증_리스트.xlsx');">엑셀다운</button>
		<c:if test="${zregion_gb eq 'A'}">
			<button class="btn-tb-top excel-2" onclick="openExcelPopup(zregion, zdept, zdept_brch, zemp, before_yyyymm, after_yyyymm, active_bcg_id, 'excelIndAuth_da.do', '[관리용]개인정보인증_리스트.xlsx');">전사 엑셀다운</button>
		</c:if>
	</div>
</div>
...
<div class="tbl">
	<table>
		<colgroup>
			...
			<col style="width: 9%;">
			<col style="width: 9%;"> <!-- 팀 -->
			<col style="width: 11%;"> <!-- 교사 -->
			<col style="width: 7%;">
			<col style="width: 11%;"> <!-- 회원 -->
			<col style="width: 6%;">
			<col style="width: 10%;"> <!-- 법정대리인관계 -->
			<col style="width: 8%;"> <!-- 법정대리인명 -->
		...
		<tbody id="crd-main-tbody">
			<tr>
				<td colspan="12" style="font-size:14px;">조회결과가 없습니다.</td>
			</tr>
		</tbody>
	</table>
